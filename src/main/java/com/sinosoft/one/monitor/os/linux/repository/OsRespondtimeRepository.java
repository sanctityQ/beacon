...
public interface OsRespondtimeRepository extends PagingAndSortingRepository<OsRespondtime, String> {
	...
	//根据时间
	@Query("from OsRespondtime o where o.sampleDate between str_to_date(?2,?4) and str_to_date(?3,?4) and o.os.osInfoId= ?1 ORDER by o.sampleDate")
	public List<OsRespondtime> findOsRespondTimeByDate(String osid,String beginTime,String endTime,String dateFormat);
	
	//物理内存利用率最大值
	@SQL("select MAX(RESPOND_TIME) from GE_MONITOR_OS_RESPONDTIME where SAMPLE_DATE between str_to_date(?2,?4) and str_to_date(?3,?4) and OS_INFO_ID= ?1 ")
	public String findMaxRespondTime(String osInfoId,String begin,String end,String dateFormat);
	
	//物理内存内存利用率最小值
	@SQL("select MIN(RESPOND_TIME) from GE_MONITOR_OS_RESPONDTIME where SAMPLE_DATE between str_to_date(?2,?4) and str_to_date(?3,?4) and OS_INFO_ID= ?1 ")
	...
	//获取离轮询间隔内最近一次的记录
	@SQL("select * from GE_MONITOR_OS_RESPONDTIME o where o.SAMPLE_DATE=(select max(SAMPLE_DATE) from GE_MONITOR_OS_RESPONDTIME where SAMPLE_DATE between str_to_date(?3,?4) and str_to_date(?2,?4) and OS_INFO_ID =?1) and o.OS_INFO_ID = ?1")
	...
	@SQL("select RESPOND_TIME from GE_MONITOR_OS_RESPONDTIME where SAMPLE_DATE between str_to_date(?2,?4) and str_to_date(?3,?4)")
