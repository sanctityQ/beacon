...
public interface OsStatiRepository extends PagingAndSortingRepository<OsStati, String> {
	...
	@SQL("delete from GE_MONITOR_OS_STATI o where o.RECORD_TIME = str_to_date(?3,?4) and o.TYPE= ?2 and o.OSID= ?1")
	public void deleteStatiRmThisHour(String osInfoId,String type,String recordTime ,String dateFromat);
	
	//一个小时点统计表查询
	@Query("from OsStati o where o.recordTime = str_to_date(?3,?4) and o.type= ?2 and o.osid= ?1")
	public OsStati getStatiThisTime(String osInfoId,String type,String recordTime ,String dateFromat);
	
	
	//时间段内的所有某类型记录
	@Query("from OsStati o where o.recordTime between str_to_date(?3,?5) and str_to_date(?4,?5) and o.type= ?2 and o.osid= ?1 ORDER by o.recordTime")
	public  List<OsStati> findStatiByTimeSpan(String osInfoId,String type,String begin ,String end, String dateFromat);
	
	

	//时间段内的最大最小品均值
	@SQL("select Max(AVERAGE_VALUE) \"maxAvgValue\" ,Min(AVERAGE_VALUE) \"minAvgValue\", avg(AVERAGE_VALUE) \"avgValue\"  from GE_MONITOR_OS_STATI where RECORD_TIME between str_to_date(?3,?5) and str_to_date(?4,?5) and TYPE= ?2 and OSID= ?1 ")
	public StatiDataModel  findStatiMaxMinAvgByTimeSpan(String osInfoId,String type,String begin ,String end, String dateFromat);
	
	
	//时间段内的所有统计记录
	@SQL("select max(max_value) \"maxValue\" ,min(min_value) \"minValue\" ,avg(AVERAGE_VALUE) \"avgValue\" ,max(AVERAGE_VALUE) \"maxAvgValue\" ,min(AVERAGE_VALUE) \"minAvgValue\" ,str_to_date(date_format(o.record_time,'%Y-%m-%d'),'%Y-%m-%d') \"date\" from GE_MONITOR_OS_STATI o where  o.OSID= ?1 and o.TYPE =?2 and  RECORD_TIME BETWEEN str_to_date(?3,?5) and str_to_date(?4 ,?5) group by date_format(o.record_time,'%Y-%m-%d') order by date_format(o.record_time,'%Y-%m-%d')")
