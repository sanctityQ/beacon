...
public interface OsAvailabletempRepository extends PagingAndSortingRepository<OsAvailabletemp, String> {
	...
	//根据时间
	@SQL("select * from GE_MONITOR_OS_AVAILABLETEMP o where o.SAMPLE_DATE between str_to_date(?2,?4) and str_to_date(?3,?4) and o.OS_INFO_ID= ?1 ORDER by o.SAMPLE_DATE")
	public List<OsAvailabletemp> findOsAvailabletempByDate(String osid,String beginTime,String endTime,String dateFormat);

	//根据时间和状态
	@SQL("select * from GE_MONITOR_OS_AVAILABLETEMP o where o.SAMPLE_DATE between str_to_date(?2,?4) and str_to_date(?3,?4) and o.OS_INFO_ID= ?1 and o.status= ?5 ORDER by o.SAMPLE_DATE")
...
//	@SQL("select * from GE_MONITOR_OS_AVAILABLETEMP o where o.sample_date=(select max(sample_date) from GE_MONITOR_OS_AVAILABLETEMP where sample_date<to_date(?2,?3)) and o.OS_INFO_ID= ?1")
	@SQL("select * from GE_MONITOR_OS_AVAILABLETEMP o where o.SAMPLE_DATE=(select max(SAMPLE_DATE) from GE_MONITOR_OS_AVAILABLETEMP where SAMPLE_DATE<str_to_date(?2,?3) and OS_INFO_ID = ?1) and o.OS_INFO_ID = ?1")
...
//	统计临时表的 轮询时间改变次数
	@SQL("select count(intercycle_time) \"count\",intercycle_time \"interval\" from ge_monitor_os_availabletemp where OS_INFO_ID= ?1 and SAMPLE_DATE > str_to_date(?2,?3) group by intercycle_time")
