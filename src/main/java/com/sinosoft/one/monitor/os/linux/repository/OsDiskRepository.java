...
public interface OsDiskRepository extends PagingAndSortingRepository<OsDisk, String> {
	...
	//根据时间
	@Query("from OsDisk o where o.sampleDate between str_to_date(?2,?4) and str_to_date(?3,?4) and o.os.osInfoId= ?1 ORDER by o.sampleDate")
	public List<OsDisk> findOsDiskByDate(String osid,String beginTime,String endTime,String dateFormat);

	//磁盘利用率最大值
	@SQL("select MAX(USED_UTILI_ZATION) from GE_MONITOR_OS_DISK where SAMPLE_DATE between  str_to_date(?2,?4) and str_to_date(?3,?4) and OS_INFO_ID= ?1 ")
	public String findMaxDiskUtilZation(String osInfoId,String begin,String end,String dateFormat);
	
	//磁盘利用率最小值
	@SQL("select MIN(USED_UTILI_ZATION)  from GE_MONITOR_OS_DISK where SAMPLE_DATE between  str_to_date(?2,?4) and str_to_date(?3,?4) and OS_INFO_ID= ?1 ")
	...
	@SQL("select * from GE_MONITOR_OS_DISK o where o.SAMPLE_DATE=(select max(SAMPLE_DATE) from GE_MONITOR_OS_DISK where SAMPLE_DATE between str_to_date(?2,?4) and str_to_date(?3,?4) and OS_INFO_ID = ?1) and o.OS_INFO_ID = ?1 ORDER by o.SAMPLE_DATE")
